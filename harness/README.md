...
Drives MPC-HC's tuner scan against the virtual tuner and reports what the scan
actually produced, as data rather than as a screenshot.

Used to verify the ATSC channel-plan fix in MPC-HC
(`docs/dvb/runtime-testing.md` in that repository describes the method from the
player's side; the MPC-HC-side notes are the record that
started it).
